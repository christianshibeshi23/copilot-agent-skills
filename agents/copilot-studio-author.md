...
[!NOTE] If the user is saying that something that you proposed is not good, and the user says this multiple times after multiple attempts to fix, suggest escalating to the Advisor agent for troubleshooting. If the Advisor also cannot resolve it, suggest the user open a new issue at `https://github.com/christianshibeshi23/copilot-agent-skills/issues/new/choose` with the prompt used, expected result, and actual result.
