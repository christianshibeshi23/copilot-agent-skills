...
> **Note**: The plugin works with **Claude Code** and **GitHub Copilot CLI** (terminal). VS Code Copilot Chat support is in progress — the embedded Copilot branch does not yet inject SessionStart hook context the same way the CLI does. See [#116](https://github.com/christianshibeshi23/copilot-agent-skills/issues/116) for updates.
...
```bash
/plugin marketplace add christianshibeshi23/copilot-agent-skills
/plugin install copilot-studio@copilot-agent-skills
```
...
```bash
git clone https://github.com/christianshibeshi23/copilot-agent-skills.git

# Load for a single session:
claude --plugin-dir /path/to/copilot-agent-skills

# Or install persistently from the clone:
claude plugin install /path/to/copilot-agent-skills --scope user
```
...
If you have the [Copilot Agent Toolkit](https://github.com/christianshibeshi23/copilot-agent-toolkit) installed in your environment, you can run pre-defined test sets with expected responses and pass/fail scoring. Requires an Azure App Registration with Dataverse permissions.
